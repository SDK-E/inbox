...
Most candidates are mixed. Sort each _section_, not each file: a design skill is usually process in its audit steps and taste in its token values, and taking the whole file imports someone else's taste along with their method.
...
Check for sibling dependencies _before_ installing, not after: grep the candidate for relative paths and skill names, and confirm each one resolves in your bundle.
