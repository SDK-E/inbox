...
const DocumentationSyncDetection: Plugin = async ({ client }) => {
  ...
  return {
    event: async ({ event }) => {
      ...
      const docCandidates = docRoots.flatMap((root) => [root, `${root}${file.split("/").pop()?.replace(/\.[^.]+$/, ".md") ?? ""}`])
...
export default { id: "documentation-sync-detection", server: DocumentationSyncDetection }
