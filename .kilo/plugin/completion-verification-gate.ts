  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      if (
        ...
        output.args !== null &&
        "command" in output.args
      ) {
        const cmd = String(output.args.command);
        if (
          /\b(git commit|git push|next build|pnpm build)\b/.test(cmd) &&
          !verified.has(input.sessionID)
        ) {
          await client.app.log({
            body: {
              service: "completion-verification-gate",
          ...
          throw new Error(
            "Run required verification (pnpm check, pnpm test) before completing",
          );
        }
      }
    },
...
export default {
  id: "completion-verification-gate",
  server: CompletionVerificationGate,
};
